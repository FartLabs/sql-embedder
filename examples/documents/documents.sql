-- documentsTable is a table for search documents with vector embeddings and multi-tenant support
...
-- Index on tenant_id and world_id for efficient filtering
...
-- documentsDeleteWorld
-- Delete all documents for a world within a tenant
DELETE FROM
  search_documents
WHERE
  tenant_id = ?
...
-- documentsDelete
-- Delete a specific document by id, tenant, and world
DELETE FROM
  search_documents
WHERE
  id = ?
...
-- documentsUpsert
-- Insert or replace a document with embedding
INSERT
...
-- documentsSearch
-- Hybrid search using RRF (Reciprocal Rank Fusion) combining FTS and vector search
WITH vec_matches AS (
  SELECT
    id AS rowid,
...
-- documentsSearchByWorlds
-- Hybrid search with world filtering using RRF
WITH vec_matches AS (
  SELECT
    id AS rowid,
    row_number() OVER (PARTITION BY NULL) AS rank_number
