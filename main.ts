  ...
  for await (
    ...
    for (const child of ast.rootNode.children) {
      if (child.type === "comment") {
        ...
        statements.push({ sql: child.text, comments: pendingComments });
        pendingComments = [];
      }
    }
    ...
    for (const statement of statements) {
      ...
      if (!variableName) {
        console.warn(
          `No valid preceding comment for statement in ${entry.path}. Add "-- variableName" above (description optional). Skipping.`,
          statement,
        );
        continue;
      }
