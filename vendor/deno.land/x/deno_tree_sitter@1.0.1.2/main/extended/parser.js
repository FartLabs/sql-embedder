...
import {
  _childrenWithSoftNodes,
  ExtendedTree,
  HardNode,
} from "./node_extended.js";
...
export async function createParser(
  wasmUint8ArrayOrFilePath,
  { disableSoftNodes = false, moduleOptions } = {},
) {
  // download if given a url
  if (
    typeof wasmUint8ArrayOrFilePath == "string" &&
    wasmUint8ArrayOrFilePath.match(/^https?:\/\//)
  ) {
    wasmUint8ArrayOrFilePath = await fetch(wasmUint8ArrayOrFilePath).then(
  ...
  if (wasmUint8ArrayOrFilePath instanceof Uint8Array) {
    const hashString = Array.from(
      new Uint8Array(
        await crypto.subtle.digest("SHA-256", wasmUint8ArrayOrFilePath),
      ),
