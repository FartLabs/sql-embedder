...
export default async function (Module) {
  ...
  var getMemory = /* @__PURE__ */ __name((size) => {
    ...
    ___heap_base = end // fix race condition that seems to only show up when loaded in the browser (not this was carried over from older loader so unsure if race condition still exists)
  ...
  async function createWasm() {
    ...
    try {
      ...
      //
      if (
        !wasmBinary && typeof WebAssembly.instantiateStreaming == "function" &&
        !isDataURI(binaryFilePath) && isUrl(binaryFilePath) &&
        !isFileURI(binaryFilePath) && fetch
      ) {
        try {
          var response = fetch(binaryFilePath, {
            credentials: "same-origin",
          });
          instantiationResult = await WebAssembly.instantiateStreaming(
            response,
            imports,
          );
        } catch (reason) {
      ...
      if (!instantiationResult) {
        try {
          ...
          // instantiate
          //
          instantiationResult = await WebAssembly.instantiate(
            binaryArray,
            imports,
          );
        } catch (reason) {
  ...
  var _ts_lookahead_iterator_new =
  ...
  var _ts_lookahead_iterator_next =
  ...
  var _ts_query_is_pattern_rooted =
  ...
  var _ts_node_prev_sibling_wasm =
